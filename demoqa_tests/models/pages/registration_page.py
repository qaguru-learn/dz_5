...
class RegistrationPage:
    ...
    def fill_state_city(self, current_user: User):
        browser.element('#react-select-3-input').should(be.blank)\
            .type(current_user.state_city.split(' ')[0]).press_enter()
        browser.element('#react-select-4-input').should(be.blank)\
            .type(current_user.state_city.split(' ')[-1]).press_enter()
        return self
